...
The Schmidt (2007) dataset contains many variables, and this vignette focuses
on the displayed subset of the variables:
...
task behavior, `Session_number` which specifies the within-session observation ordering,
`Outcome` which contains the outcome data,`Condition` which specifies whether 
the outcome is in the baseline ("A") phase or the treatment ("B") phase, 
`Phase_num` which specifies whether the data is in the first or second phase
pair, `Metric` which specifies whether it is percentage or count data,
`Session_length` which specifies the length of the observation session,
 `direction` which specifies the improvement direction, and `n_Intervals`
...
While there are a number of possible arguments for the function, let's first focus
on the minimum necessary arguments for using the batch calculator function.

The argument `dat` should be the dataframe containing all of the observations 
for all of the SCDs of interest. 

The `grouping` argument should the set of variables that uniquely identify each
series. For several SCDs within a single study like the McKissick dataset, this might simply be a variable name that identifies the participant pseudonym. These variables
can be entered as bare variable names.
...
had "A" as baseline and "B" as treatment, it would be important to pick one or
the other and clean your data appropriately before using the `batch_calc_ES()`
function.

All of the other arguments are optional, and we'll introduce them slowly as we go along.

## Some Examples

Remember that data contains a pseudonym for each case (`Case_pseudonym`), a variable (`Condition`) corresponding to the baseline ("A")
...
is the direction of therapeutic improvement, so as with the `calc_ES()` function
we'll need to specify that using the `improvement` argument.

```{r}
batch_calc_ES(dat = McKissick, grouping = Case_pseudonym, condition = Condition,
              outcome = Outcome, improvement = "decrease")
```
Note that all of the arguments related to variable names are bare, or unquoted
variable names. Any argument in batch_calc_ES that accepts a variable accepts a
bare variable name, or in the case of the `grouping` argument it will accept a
vector of variable names (more on this later).

We can also request the effect sizes in a wide format:

```{r}
batch_calc_ES(dat = McKissick, grouping = Case_pseudonym, condition = Condition,
              outcome = Outcome, improvement = "decrease", format = "wide")
```
...
```{r}
batch_calc_ES(dat = McKissick, grouping = Case_pseudonym, condition = Condition,
              outcome = Outcome, ES = c("PND", "PEM", "LRRd", "SMD"), improvement = "decrease")
batch_calc_ES(dat = McKissick, grouping = Case_pseudonym, condition = Condition,
              outcome = Outcome, ES = "NOM", improvement = "decrease")
batch_calc_ES(dat = McKissick, grouping = Case_pseudonym, condition = Condition,
              outcome = Outcome, ES = "parametric", improvement = "decrease",
              warn = FALSE)
batch_calc_ES(dat = McKissick, grouping = Case_pseudonym, condition = Condition,
              outcome = Outcome, ES = "all", improvement = "decrease",
              warn = FALSE)
```

Note that the `warn` argument defaults to true. If you ask for the LOR effect
size and some of your series are not proportions or percentages, the function
will warn you that data specified as count, rate, or other will be returned as
NA for the LOR. By default, the batch calculator specifies the outcome scale as
"other", so when specifying `ES = "all"` or `ES = "parametric"` if you haven't
specified another outcome scale, you will receive that warning. You can turn off this
warning by setting `warn = FALSE` as we have done above.

As we have shown with the `improvement` argument, many arguments such as `improvement` `scale`, `intervals`, and `observation_length` argument can 
be specified like their parallel arguments for the `calc_ES()` functions, in 
...
of intervals per observation, and  length of the observation sessions. 

```{r}
batch_calc_ES(dat = McKissick, grouping = Case_pseudonym, condition = Condition,
              outcome = Outcome, improvement = "decrease", scale = Procedure,
              observation_length = Session_length)
```

If you are providing these inputs as variables, any series where the information is not
...
```{r}
batch_calc_ES(dat = McKissick, grouping = Case_pseudonym, condition = Condition,
              outcome = Outcome, ES = c("LRRd", "SMD"), improvement = "decrease",
              scale = Procedure, observation_length = Session_length)
batch_calc_ES(dat = McKissick, grouping = Case_pseudonym, condition = Condition,
              outcome = Outcome, ES = c("LRRd", "SMD"), improvement = "decrease",
              scale = Procedure, observation_length = Session_length,
              std_dev = "pool", bias_correct = FALSE)
```

Note that arguments common to several functions, such as `bias_correct`
...
Note that for the `grouping` argument we specified a vector of bare variable
names, one for the case name, one for the behavior type, and one for which phase
pair the data came from. Each unique combination of those three variables
represents a different data series we want to estimate effect sizes for. This
could also be used for other types of groupings, such as if we wanted effect
sizes for each case across several different studies We have specified
`session_number`, although this is not strictly necessary for the requested set
of effect sizes. We have set the `baseline_phase` argument just to be sure that
the baseline phase is always specified correctly. Because the direction of
therapeutic improvement varies by series, we have specified `improvement =
direction`, because the `direction` variable says whether improvement should be
calculated as "increasing" or "decreasing". We have also specified a variable
for `scale` because verbal outbursts were measured as a count and time on task
was measured as a proportion. We have also specified the number of `intervals`
for those series which were measured as a proportion, and the
`observation_length` argument for those series which were measured as a count.
