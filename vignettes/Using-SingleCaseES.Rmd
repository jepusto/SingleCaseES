...
This function has a bunch of arguments, but several of them are optional and only used for certain effect size metrics. For the moment, let's focus on the first few arguments, which are all we need to get going:
...
and standardize the coding before using the `batch_calc_ES()` function.

All of the other arguments are optional, and we'll introduce them as we go
along. 
...
```{r}
batch_calc_ES(dat = McKissick, grouping = Case_pseudonym, 
              condition = Condition,
              outcome = Outcome, 
...
quotes). All of the arguments in `batch_calc_ES` that accept a variable should be specified using bare
variable names.
...
Use the `ES` argument to specify which effect size index or indices to
calculate: 
...
This argument works exactly like `calc_ES()`. It also allows you to specify all
effect sizes (`ES = "all"`), parametric effect sizes (`ES = "parametric"`), or non-overlap measures (`ES = "NOM"`):
...
size and some of the data series are not proportions or percentages, the
function will warn you that data specified as count, rate, or other will be
returned as NA for the LOR. By default, the batch calculator specifies the
outcome scale as "other". If you've specified `ES = "all"` or `ES =
"parametric"`, you'll need to specify an outcome scale (or a variable
identifying the outcome scale) to avoid receiving that warning. You can turn off
this warning by setting `warn = FALSE`, as we have done above.

As we have shown with the `improvement` argument, arguments such as
`scale`, `intervals`, and `observation_length` can be
specified like their parallel arguments for the `calc_ES()` functions, in that
it will accept a uniform value (such as `improvement = "decrease"` or `scale =
"proportion"`) if all of the series within the dataset are uniform with respect
towards a particular argument. However, if the values indicated by these
arguments varies from series to series, it is also possible to specify the name
of a variable which identifies the direction of improvement, outcome scale,
number of intervals per observation, and  length of the observation sessions.
