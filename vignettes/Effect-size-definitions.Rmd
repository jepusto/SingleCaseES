...
@bonett2020interval proposed the log ratio of medians (LRM) effect size to quantify the change in the medians from phase A to phase B. It is the natural logarithm of the ratio of medians. This effect size is appropriate for outcomes that are skewed or right-censored [@bonett2020interval]. For an outcome where increase is desirable, the LRM parameter is defined as

$$
\theta = \ln\left(\eta_B / \eta_A\right),
$$
where $\eta_B$ and $\eta_A$ are the population median for phase B and phase A, respectively.

### Estimation
A basic estimator of the $\theta$ is given by
...
### Standard errors and confidence intervals
Under the assumption that the outcome data in each phase are mutually independent, based on the fact that the logarithm of the median is the same or close to the median of the log-transformed outcomes, the standard error for $LRM$ is given by

$$
SE_{LRM} = \sqrt{\frac{\ln(y_{o2})-\ln(y_{o1})}{2\ z_{\alpha^*/2} }},
$$
[@bonett2020confidence] where $o1 = \text{max} (n/2 - \sqrt{n}, 1)$, $o2 = n - o1 + 1$, and $\alpha^*/2 = 0.5^n \sum_{i=0}^{o1-1}[n!/(i!(n-i)!]$. $y_{o2}$ and $y_{o1}$ are the $o2_\text{th}$ and $o1_\text{th}$ outcome for the sorted phase B or phase A data.

The confidence interval for $\theta$ is 

$$
[LRM - z_{\alpha/2} \times SE_{LRM},\quad LRM + z_{\alpha/2} \times SE_{LRM}],
$$
