...
## Tau-BC
@tarlow2017improved proposed the Tau-BC effect size, which is a variation of Tau but includes a baseline trend correction. The method first tests whether the slope of the baseline trend is statistically significant based on Kendall's rank correlation. If the baseline slope is significantly different from zero, the outcome data are adjusted for baseline trend using Theil-Sen regression, and the residuals from Theil-Sen regression are used to calculate the Tau (non-overlap) index. If the baseline slope is not significantly different from zero, then no baseline trend adjustment is made, and the Tau-BC effect size is the same as Tau.
