...
After estimating the phase A time trend, $\tau_{BC}$ is estimated by de-trending the full data series and calculating Tau on the de-trended observations. Specifically, set $\hat\epsilon_i^A = y_i^A - \hat\beta (i) - \hat\alpha$ for $i = 1,...,m$ and $\hat\epsilon_j^B = y_j^B - \hat\beta (m + j) - \hat\alpha$. For an outcome where increase is desirable, calculate 
...
If calculated with Kendall's rank correlation, Tau-BC is estimated as the rank correlation between $\hat{\boldsymbol{\epsilon}} = \left\{ \hat\epsilon^A_1, \dots, \hat\epsilon^A_m, \hat\epsilon^B_1, \dots, \hat\epsilon^B_n \right\}$ and a dummy coded variable $\boldsymbol{q} = \left\{0_1,\dots,0_m, 1_1,\dots,1_n \right\}$. When there are no ties in $\hat{\boldsymbol{\epsilon}}$ and $\boldsymbol{q}$,
$$
\text{Tau}_{BC} = \frac{S}{D} = \frac{\sum_{s<t} sign(\hat\epsilon_s-\hat\epsilon_t)sign(q_s-q_t)}{\frac{(m+n)(m+n-1)}{2}},
$$
where $s = 1,\dots, (m+n)$ and $t = 1,\dots, (m+n)$. When there are ties, $D$ is calculated with adjustment for ties [@kendall1970rank, p. 35]. Let $U = \frac{1}{2}\sum_u u(u-1)$ and $V = \frac{1}{2}\sum_v v(v-1)$, where $u$ and $v$ represent the number of tied consecutive values in sorted $\hat{\boldsymbol{\epsilon}}$ and $\boldsymbol{q}$, respectively.
$$
D_{\text{adjusted}} = \sqrt{\frac{(m+n)(m+n-1)}{2}-U} \sqrt{\frac{(m+n)(m+n-1)}{2}-V}.
$$

Then Tau-BC (Kendall, adjusted for ties) is estimated by
$$
\text{Tau}_{BC}^* = \frac{S}{D_{\text{adjusted}}} = \frac{\sum_{s<t} sign(\hat\epsilon_s-\hat\epsilon_t)sign(q_s-q_t)}{\sqrt{\frac{(m+n)(m+n-1)}{2}-U} \sqrt{\frac{(m+n)(m+n-1)}{2}-V}}.
$$

### Standard errors and confidence intervals

The exact sampling distribution of $\text{Tau}_{BC}$ (Kendall's rank correlation) has not been described. @tarlow2017improved proposed to approximate its sampling variance using the conservative estimator
$$
