...
check_single <- function(ES, ES_family, A_data, B_data, Kendall = FALSE) {
  ...
  if (ES == "Tau_BC" & Kendall == TRUE) {
    tau_calculation <- "Kendall"
  } else if (ES == "Tau_BC" & Kendall == FALSE) {
    tau_calculation <- "Nlap"
  }
  ...
  app$setInputs(SCD_es_calculator = "Calculator")
  app$setInputs(A_dat = toString(A_data))
  app$setInputs(B_dat = toString(B_data))
  app$setInputs(ES_family = ES_family)
